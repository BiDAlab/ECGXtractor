...
base_path = "datasets/ptb/"
multi_patients_path = "datasets/ptb/multi_patients.txt"
healthy_patients_path = "datasets/ptb/healthy_patients.txt"


def get_list_of_patients(keyword):
    with open(healthy_patients_path, "r") as fid:
        h_pats = fid.readlines()
    h_pats = [p[:-1] for p in h_pats]
    with open(multi_patients_path, "r") as fid:
        m_pats = fid.readlines()
    m_pats = [p[:-1] for p in m_pats]
    if   keyword == 'healthy':                  return h_pats
    elif keyword == 'multi':                    return m_pats
    else:
        pats = [name for name in os.listdir(base_path) if not os.path.isfile(os.path.join(base_path, name))]
        pats = [p for p in pats if not p == 'preproc_csv']
        if   keyword == 'all':                  return pats
        elif keyword == 'all-healthy':          return [p for p in pats if p not in h_pats]
        elif keyword == 'all-multi':            return [p for p in pats if p not in m_pats]
        elif keyword == 'all-healthy-multi':    return [p for p in pats if p not in h_pats and p not in m_pats]
...
do_everything('healthy', 'single', only_test_split, 'verification', 'healthy_single_verification.json')
do_everything('multi', 'single', only_test_split, 'verification', 'multi_single_verification.json')
# do_everything('multi', 'multi', only_test_split, 'verification', 'multi_multi_verification.json')
do_everything('healthy', 'single', classic_split, 'identification', 'healthy_single_identification.json')
do_everything('multi', 'single', classic_split, 'identification', 'multi_single_identification.json')
do_everything('multi', 'multi', autoencoder_split, 'identification', 'multi_multi_identification.json')