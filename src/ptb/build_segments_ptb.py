...
segment_back = 160
segment_length = 400
consecutive_heartbeats = 10  # -1 for template, 1 for single hb, otherwise specify the length of cons. blocks (es. 10)
best_number = 5  # default 5
base_path = "datasets/ptb/preproc_csv"
base_path_segment = "datasets/ptb"
...
with open("datasets/ptb/r_peaks.json", 'r') as fid:
...
for k, v in tqdm.tqdm(dict_r_peaks.items()):
    ...
    segments = [s for s in segments if s.shape == (400, 12)]
